...
const firework = () => {
    ...
    // Canvas
    const canvas = document.querySelector('canvas.firework_gl')
    ...
    const createFirework = (count, position, size, texture, radius, color) => {
        ...
        const material = new THREE.ShaderMaterial({
            ...
            uniforms: {
                ...
                uResolution: new THREE.Uniform(sizes.resolution), // fix proportion to render height issue
                uTexture: new THREE.Uniform(texture),
                uColor: new THREE.Uniform(color),
                uProgress: new THREE.Uniform(0),
                // uTime: new THREE.Uniform(0),
                // uCutoff: new THREE.Uniform(0.5),
        ...
        gsap.to(
            ...
            { value: 1, duration: 5, ease: "linear", onComplete: destroy }
        )
    }
    ...
    gui.add(skyParameters, 'elevation').min(-3).max(10).step(0.1).onChange(updateSky) 
    gui.add(skyParameters, 'azimuth').min(-180).max(180).step(0.1).onChange(updateSky)
    gui.add(skyParameters, 'exposure').min(0).max(1).step(0.0001).onChange(updateSky)
